...
from ..models import Suite, SuiteSetCase, ReportItem, Report, Task
...
def reports_top_10_slow(request):
    ...
    resp = []
    for i in rp:
        if i.result == Element.CASE_CHECK_SUCCESS:
            result = "通过"
        elif i.result == Element.CASE_CHECK_FAIL:
            result = "失败"
        else:
            result = "无检查点"
...
def reports_top_slow_export(request):
    ...
    for i in item_entry:
        ...
        data.append({"url": url, "params": i.params, "name": i.name, "method": i.method, "hope": i.hope,
                     "result": result, "sum_time": i.sum_time, "fact": i.fact, "id": i.id})
        ...
        for j in data:
            ...
            worksheet.write("I" + str(temp), j["id"])
            temp += 1
        workbook.close()

    file_name1 = os.path.join(settings.BASE_DIR, "TestModel/Report", name + ".xlsx")
...
def report_task(request):
    ...
    for i in report:
        ...
        # 只统计测试完成后的任务
        if ta.task_state == Element.TASK_STATE_FINISH:
            xAxis_data.append(i.name)
            if i.sum_pass:
                sum_pass.append(i.sum_pass)
            if i.sum_fail:
                sum_fail.append(i.sum_fail)

    resp["xAxis_data"] = xAxis_data
    resp["sum_pass"] = sum_pass
    resp["sum_fail"] = sum_fail
...
def reports_list(request):
    ...
    for i in reports_pagi.object_list:
        task_entry = Task.objects.get(id=i.task_id)
        suite_entry = Suite.objects.get(id=task_entry.suite_id)
        response["list"].append(
            {"name": i.name, "id": i.id, "sum_time": i.sum_time, "sum_pass": i.sum_pass, "start_time": i.start_time,
             "sum_fail": i.sum_fail, "suite_name": suite_entry.name}
        )
...
def report_detail_del(request):
    ...
    try:
        ...
        if task_entry.task_state == 2:
            report_entry.delete()
            file_name1 = os.path.join(settings.BASE_DIR, "TestModel/Report", report_entry.name + ".xlsx")
