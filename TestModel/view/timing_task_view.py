...
from ..models import Task, Suite, Report
...
def get_timing_task_list(request):
    ...
    task_type = Element.TASK_TYPE_TIME
    if query:
...
def new_timing_task(request):
    ...
    name = data.get("name")
    task_state = Element.TASK_STATE_DEFAULT
    task_type = Element.TASK_TYPE_TIME
    suite_id = data.get("suite_id")
    # start_time2 = datetime.now().strftime("%H-%M-%S")
    ...
    scheduler.add_job(ApiTask.background_task, 'cron', hour=hour, minute=minute,
                      kwargs={"suite_id": suite_id, "task": ta, "lg_session": res_session}, id=str(ta.id))
